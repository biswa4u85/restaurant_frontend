...
export function Cart() {
  ...
  const getData = async () => {
    let orders = await apiGetCall(
      `de_restaurant_backend.api.v_0_1.order.get_current_order?customer_id=${users?.customer_id}`,
      { token: `Basic ${users.auth_key}` }
    );
    ...
    if (type == "add") {
      if (item.item_name in items) {
        items[item.item_name] = {
          ...
          count: Number(items[item.item_name].count) + 1,
          rate: item.rate,
        };
      } else {
        items[item.item_name] = {
  ...
  const placeOrder = async () => {
    if (cart.items) {
      ...
      let orders = await apiPostCall(
        `de_restaurant_backend.api.v_0_1.order.place_order`,
        { ...params, token: `Basic ${users.auth_key}` }
      );
      if (orders.status_code == 200) {
        let confirmOrder = await apiPostCall(
          `de_restaurant_backend.api.v_0_1.cart.confirm_order`,
          { order_id: orders.order_id, token: `Basic ${users.auth_key}` }
        );
  ...
  return (
    <>
      <div className="pt-3 gurdeep-osahan-inner-header border-bottom w-100">
        <div className="row">
          <div className="col-10">
            <NavLink to="/details" className="text-dark fw-bold">
              <i className="btn_detail fa fa-chevron-left"></i>Review Order
            </NavLink>
          </div>

          <div className="col-2">
            <NavLink to="/order-summary" className="profile-icon">
      ...
      <div>
          ...
          <div className="row align-items-center unbilled">
            ...
            <div className="col mt-3 ml-0 ps-0 ">
              <p className=" float-start">
                GRAND TOTAL <br />
                {cart?.total} + {cart.total < 2 ? "item" : "items"} served{" "}
              </p>
            </div>
            <div className="col ">
              <NavLink to="/home">
                <h6 className="float-right fw-bolder mt-3 ">
                  {" "}
                  {"Pending"}
                  <img
                    src="https://restaurant.scrollmonkey.com/files/Vector (3).png"
                    alt=""
                  />
                </h6>
              </NavLink>
        ...
        {cart?.items &&
          Object.keys(cart.items).map((menu, k) => (
            <section key={k} className="p-3 border-bottom">
              <div className="rounds m-3 mt-4">
                <h6 className="round-no">Item #{Number(k) + 1}</h6>
                <div className="row mt-3">
                  <div className="col-8">
                    <img
                      src="https://restaurant.scrollmonkey.com/files/veg.png"
                      alt=""
                    />
                    ...
                    <div className=" input-group inline-group shadow-sm float-right ">
                      <div className="input-group-prepend">
                        <button
                          ...
                          onClick={() => updateCart(cart.items[menu], "minus")}
                        >
                          <img
                            src="https://restaurant.scrollmonkey.com/files/minus-dark.png"
                      ...
                      <input
                        ...
                        value={Number(cart.items[menu].count)}
                      />
                      <div className="input-group-append">
                        <button
                          className="btn btn-outline-secondary btn-plus"
                          onClick={() => updateCart(cart.items[menu], "add")}
                        >
                          <img
                            src="https://restaurant.scrollmonkey.com/files/plus-dark.png"
                            alt=""
                          />
                        </button>
                      </div>
                ...
                <div className="row mt-2">
                  <div className="col-8">
                    <h6 className="fw-bolder">
                      ₹{Number(cart.items[menu].rate).toFixed(2)}
                    </h6>
                  </div>
                  <div className="col-4">
                    <h6 className="fw-bolder float-right">
                      ₹
                      {(
                        Number(cart.items[menu].rate) *
                        Number(cart.items[menu].count)
                      ).toFixed(2)}
                    </h6>
                  </div>
                </div>
      ...
      <section className="border-bottom add-more p-3">
        <NavLink to="/home" className={"p-3"}>
          ...
          <img
            src="https://restaurant.scrollmonkey.com/files/Vector (4).png"
            alt=""
        ...
        {isnote ? (
          <>
            <textarea
              ...
              onChange={(obj) => addingNotes(obj.target.value)}
              style={{ width: "100%" }}
            />
          </>
        ) : (
          <span onClick={() => setIsnote(true)} className="p-3">
            <img
              src="https://restaurant.scrollmonkey.com/files/Vector (12).png"
              alt=""
            />
      ...
      <div className="place-order  p-3 m-3 mb-5 shadow-sm ">
        ...
        <div className="row pb-3 mt-3">
          ...
          <div className="col-4">
            <h5 className="fw-bolder float-right">
        ...
        <div className="mt-2">
          <button
            ...
            className="btn save-btn place-order-btn btn-block box_rounded w-100 py-3"
          >
            Place Order
          </button>
        </div>
        <div className="row mt-4">
          ...
          <div className="col-4">
            <h5 className="fw-bolder float-right">
              ₹
              {Number(cart?.price + orders[0]?.payment[0]?.grand_total).toFixed(
                2
              )}
            </h5>
          </div>
      ...
      {/* <Modal
        ...
        showCloseIcon={false}
        onClose={() => setPlaceOrderClicked(true)}
        center
      >
        <div className="text-center p-4">
          <h3 className="fw-bold d-inline text-success">Success</h3>
          <div>
            <img src={OKIcon} alt="My animated GIF" />
          </div>
          <h5 className="fw-bolder">Your order is confirmed!</h5>
          <p>
            Please wait upto 20mins for your order to be served fresh. Thank you
            for your patience.
          </p>
        </div>
      </Modal> */}
        ...
        >
          ...
          >
            <div className="text-center p-4">
              ...
              <h5 className="fw-bolder">Your order is confirmed!</h5>
              <p>
                Please wait upto 20mins for your order to be served fresh. Thank
